...
class MainForm(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.lineInstallationFolder.setText(
            (INI_OPTIONS.value("installation_folder", ""))
        )
        # Left side Buttons
        self.buttonThemeLight.clicked.connect(
            lambda: self.initialize_colors("light")
        )
        self.buttonThemeDark.clicked.connect(
            lambda: self.initialize_colors("dark")
        )
        self.buttonThemeDefault.clicked.connect(
            lambda: self.initialize_colors("default")
        )
        self.buttonLanguage_english.clicked.connect(
            lambda: self.initialize_language("en")
        )
        self.buttonLanguage_spanish.clicked.connect(
            lambda: self.initialize_language("es")
        )
        self.buttonLanguage_french.clicked.connect(
            lambda: self.initialize_language("fr")
        )
        self.buttonLanguage_deutsch.clicked.connect(
            lambda: self.initialize_language("de")
        )
        ...
        self.buttonArcDps_mechanics.clicked.connect(
            self.update_arcdps_mechanics
        )
        self.buttonWebsite_arcdps.clicked.connect(self.open_web_arcdps)
        self.buttonWebsite_arcdpsmechanics.clicked.connect(
            self.open_web_arcdpsmechanics
        )
        ...
        self.buttonWebsite_builds_alternative.clicked.connect(
            self.open_web_builds_alternative
        )
        ...
        self.buttonWebsite_gw2raidexplorer.clicked.connect(
            self.open_web_gw2raidexplorer
        )
        ...
        self.checkBosses.stateChanged.connect(
            lambda: self.save_checkboxes_status(self.checkBosses)
        )
        self.checkMinis.stateChanged.connect(
            lambda: self.save_checkboxes_status(self.checkMinis)
        )
        self.checkSkins.stateChanged.connect(
            lambda: self.save_checkboxes_status(self.checkSkins)
        )
        self.checkAchievements.stateChanged.connect(
            lambda: self.save_checkboxes_status(self.checkAchievements)
        )
        self.checkCurrency.stateChanged.connect(
            lambda: self.save_checkboxes_status(self.checkCurrency)
        )
        # Default vars
    ...
    def change_statusbar(self, statusmode, message):
        ...
        if statusmode == "wait":
            self.statusbar.setStyleSheet(
                "background-color: rgb(234, 140, 6);color: rgb(0, 0, 0);"
            )
            ...
            self.statusbar.setStyleSheet(
                "background-color: rgb(190, 1, 190);color: rgb(255, 255, 255);"
            )
    ...
    def api_open(self, section, **keyarguments):
        ...
        if "token" in keyarguments:
            address = (
                address + separator + "access_token=" + keyarguments["token"]
            )
        ...
        address = urllib.parse.quote(
            address, safe="/:=", encoding="utf-8", errors="strict"
        )
        if self.debug_mode:
    ...
    def check_if_ready(self):
        ...
        if len(warning) >= 1:
            ...
            self.change_statusbar(
                "ready", "Everything seems fine. Program ready."
            )
            ...
            if int(PROGRAM_VERSION) < int(online_version):
                self.change_statusbar(
                    "special",
                    "There is a new version availible, you may want to download it.",
                )
    ...
    def default_theme():
        ...
        colors = {
            "backgroundcolor": "background-color: rgb(217, 217, 217);"
            "color: rgb(0, 0, 0);",
    ...
    def dark_theme():
        ...
        colors = {
            "backgroundcolor": "background-color: rgb(60, 63, 65);"
    ...
    def light_theme():
        ...
        colors = {
            "backgroundcolor": "background-color: rgb(150, 200, 255);"
    ...
    def initialize_colors(self, theme):
        ...
        self.style_lineedits = colors[
            "inputcolorreadonly"
        ]  # For the reset of YES/NO fields
        self.load_permissions()
    ...
    def set_colors(self, widget, colors):
        ...
        for item in widget.children():
            ...
            # SPECIALS: Set style to it and then re-iterate on these items
                ...
                self.set_colors(item, colors)
            elif isinstance(item, QTabWidget) or isinstance(
                item, QStackedWidget
            ):
    ...
    def check_online_version():
        ...
        address = urllib.parse.quote(
            version_file, safe="/:=", encoding="utf-8", errors="strict"
        )
        address = urllib.request.urlopen(address).read().decode("utf8")
        return address
    ...
    def delete_api(self):
        ...
        if not self.comboSelectAPI.currentText() == "":
            if popup_delete():
                ...
                self.comboSelectAPI.clear()
                self.change_statusbar(
                    "ready", "API key deleted from local database."
                )
            else:
    ...
    def debugger(self):
        ...
        if self.debug_mode:
            self.debug_mode = False
            self.setFixedSize(
                QSize(self.width(), self.height() - self.plainDebugger.height())
            )
        else:
    ...
    def open_web_anet(self):
        ...
        self.change_statusbar(
            "ready", "Website for Arenanet API keys launched on your browser."
        )
    ...
    def open_web_builds(self):
        ...
        self.change_statusbar(
            "ready", "Website for Raid Builds launched on your browser."
        )
    ...
    def open_web_dpsreport(self):
        ...
        self.change_statusbar(
            "ready", "Website for Raid DPS Reports launched on your browser."
        )
    ...
    def open_web_killproof(self):
        ...
        self.change_statusbar(
            "ready", "Website to check Killproofs launched on your browser."
        )
    ...
    def open_web_raidar(self):
        ...
        self.change_statusbar(
            "ready", "Website for Raid Reports Raidar launched on your browser."
        )
    ...
    def open_web_arcdps(self):
        ...
        self.change_statusbar(
            "ready", "Website for ArcDps Plugin launched on your browser."
        )
    ...
    def open_web_arcdpsmechanics(self):
        ...
        self.change_statusbar(
            "ready",
            "Website for ArcDps Mechanics Addon launched on your browser.",
        )
    ...
    def open_web_dulfy(self):
        ...
        self.change_statusbar(
            "ready", "Website for Dulfy raid guides launched on your browser."
        )
    ...
    def open_web_gw2raidexplorer(self):
        ...
        self.change_statusbar(
            "ready", "Website for this tool launched on your browser."
        )
    ...
    def open_web_donate(self):
        ...
        self.change_statusbar(
            "ready", "Website to donate launched on your browser."
        )
    ...
    def check_folder_is_right(self):
        ...
        if folder == "":
            self.change_statusbar(
                "error", "You need to set your GW2 installation folder above."
            )
        elif "Guild Wars 2" not in folder:
            self.change_statusbar(
                "error", "You didn't set the folder for Guild Wars 2"
            )
        else:
    ...
    def update_arcdps(self):
        ...
        if self.check_folder_is_right():
            ...
            local_file_bt = "{0}/d3d9_arcdps_buildtemplates.dll".format(
                bin_folder
            )
            ...
            online_file_md5 = (
                "https://www.deltaconnected.com/arcdps/x64/d3d9.dll.md5sum"
            )
            # Check if exists
            try:
                if self.file_exists(local_file):
                    ...
                    address = (
                        urllib.request.urlopen(address).read().decode("utf8")
                    )
                    online_md5 = address.split(" ")[0]
                    # Compare online MD5 with local md5
                    if online_md5 == local_file_md5:
                        self.change_statusbar(
                            "ready", "NOPE, ArcDps was already updated."
                        )
                    else:
                        # Download files and replace them
                        self.change_statusbar(
                            "wait", "ArcDps is being updated..."
                        )
                        urllib.request.urlretrieve(online_file, local_file)
                        urllib.request.urlretrieve(
                            online_file_bt, local_file_bt
                        )
                        self.change_statusbar(
                            "ready",
                            "YES, there was a new version. ArcDps has been updated.",
                        )
                else:
                    # Download files
                    self.change_statusbar(
                        "wait", "ArcDps is not installed, downloading..."
                    )
                    urllib.request.urlretrieve(online_file, local_file)
                    urllib.request.urlretrieve(online_file_bt, local_file_bt)
                    self.change_statusbar(
                        "ready",
                        "YES, there was a new version. ArcDps has been Installed.",
                    )
    ...
    def update_arcdps_mechanics(self):
        ...
        if self.check_folder_is_right():
            self.change_statusbar(
                "wait", "Verifying hash file of ArcDps Mechanics Addon..."
            )
            # Files
            bin_folder = self.lineInstallationFolder.text() + "/bin64"
            ...
            try:
                if self.file_exists(local_file):
                    ...
                    if online_md5 == local_file_md5:
                        self.change_statusbar(
                            "ready",
                            "ArcDps Mechanics Addon was already updated.",
                        )
                    else:
                        # Download files and replace them
                        self.change_statusbar(
                            "wait", "ArcDps Mechanics Addon is being updated..."
                        )
                        urllib.request.urlretrieve(online_file, local_file)
                        self.change_statusbar(
                            "ready", "ArcDps Mechanics Addon has been updated."
                        )
                else:
                    # Download files
                    self.change_statusbar(
                        "wait",
                        "ArcDps Mechanics Addon is not installed, downloading...",
                    )
                    urllib.request.urlretrieve(online_file, local_file)
                    self.change_statusbar(
                        "ready", "ArcDps Mechanics Addon has been Installed."
                    )
            except Exception as e:
                self.change_statusbar(
                    "error", "Unexpected error: {0}".format(str(e))
    ...
    def launch_game(self):
        ...
        if self.check_folder_is_right():
            if self.game_is_running()[0]:
                self.change_statusbar(
                    "error", "Guild Wars 2 is already running."
                )
            else:
                self.change_statusbar("wait", "Launching Guild Wars 2...")
                filepath = self.lineInstallationFolder.text()
                game_paths = [
                    "{0}/Gw2-64.exe".format(filepath),
                    "{0}/Gw2.exe".format(filepath),
                    "{0}/gw2-64.exe".format(filepath),
                    "{0}/gw2.exe".format(filepath),
                ]
                for file in game_paths:
                    if self.file_exists(file):
                        ...
                        return
                self.change_statusbar(
                    "error", "Executable file could not be found."
                )
    ...
    def close_game(self):
        ...
        if self.check_folder_is_right():
            ...
            if running:
                game_proccess.kill()
                self.change_statusbar(
                    "ready", "Guild Wars 2 has been forced to be closed."
                )
            else:
    ...
    def load_api(self):
        ...
        if self.comboSelectAPI.currentText() == "":
            self.change_statusbar(
                "error", "You have not added any API key yet."
            )
        else:
            ...
            try:
                ...
                if len(warning) >= 1:
                    self.change_statusbar(
                        "error",
                        "Not enough permission to do that - {0}".format(
                            " - ".join(warning)
                        ),
                    )
                else:
            ...
            # Special exceptions
                ...
                if "HTTP Error 400" in str(e):
                    ...
                    self.change_statusbar(
                        "error",
                        "LAZY BUG 0001 - If you can read this, tell the programmer.",
                    )
                else:
    ...
    def load_permissions(self):
        ...
        if not self.comboSelectAPI.currentText() == "":
            ...
            if self.api_key is not None:
                ...
                try:
                    permissions = self.api_open(
                        "tokeninfo", token=self.api_key
                    )["permissions"]
                ...
                self.change_statusbar(
                    "ready", "API key not valid, probably empty or corrupted."
                )
    ...
    def fill_permissions(self, permissions):
        ...
        # Load everything
        all_sections = [
            ("account", self.linePermission_Account),
    ...
    def reset_permissions(self):
        ...
        # Clean everything
        permission_fields = (
            self.linePermission_Progression,
            self.linePermission_Wallet,
            self.linePermission_Unlocks,
            self.linePermission_PvP,
            self.linePermission_Inventories,
            self.linePermission_Guilds,
            self.linePermission_Guilds,
            self.linePermission_Characters,
            self.linePermission_Builds,
            self.linePermission_Account,
        ...
        bosses = [
            {
                "name": "vale_guardian",
                "flag": 0,
                "uiitem": self.lineRaidboss_valeguardian,
            },
            {
                "name": "spirit_woods",
                "flag": 0,
                "uiitem": self.lineRaidboss_spiritwoods,
            },
            {
                "name": "gorseval",
                "flag": 0,
                "uiitem": self.lineRaidboss_gorseval,
            },
            {"name": "sabetha", "flag": 0, "uiitem": self.lineRaidboss_sabetha},
            {
                "name": "slothasor",
                "flag": 0,
                "uiitem": self.lineRaidboss_slothasor,
            },
            {
                "name": "bandit_trio",
                "flag": 0,
                "uiitem": self.lineRaidboss_trio,
            },
            {
                "name": "matthias",
                "flag": 0,
                "uiitem": self.lineRaidboss_matthias,
            },
            {"name": "escort", "flag": 0, "uiitem": self.lineRaidboss_glenna},
            {
                "name": "keep_construct",
                "flag": 0,
                "uiitem": self.lineRaidboss_keepconstruct,
            },
            {
                "name": "twisted_castle",
                "flag": 0,
                "uiitem": self.lineRaidboss_twistedcastle,
            },
            {"name": "xera", "flag": 0, "uiitem": self.lineRaidboss_xera},
            {"name": "cairn", "flag": 0, "uiitem": self.lineRaidboss_cairn},
            {
                "name": "mursaat_overseer",
                "flag": 0,
                "uiitem": self.lineRaidboss_mursaat,
            },
            {"name": "samarog", "flag": 0, "uiitem": self.lineRaidboss_samarog},
            {"name": "deimos", "flag": 0, "uiitem": self.lineRaidboss_deimos},
            {
                "name": "soulless_horror",
                "flag": 0,
                "uiitem": self.lineRaidboss_desmina,
            },
            {
                "name": "river_of_souls",
                "flag": 0,
                "uiitem": self.lineRaidboss_riverofsouls,
            },
            {
                "name": "statues_of_grenth",
                "flag": 0,
                "uiitem": self.lineRaidboss_statues,
            },
            {
                "name": "voice_in_the_void",
                "flag": 0,
                "uiitem": self.lineRaidboss_dhuum,
            },
            {
                "name": "conjured_amalgamate",
                "flag": 0,
                "uiitem": self.lineRaidboss_conjureda,
            },
            {
                "name": "twin_largos",
                "flag": 0,
                "uiitem": self.lineRaidboss_twinlargos,
            },
    ...
    def reset_bosses(self):
        ...
        # Clean everything
        raids_fields = (
            self.lineRaidboss_valeguardian,
            self.lineRaidboss_spiritwoods,
            self.lineRaidboss_gorseval,
            self.lineRaidboss_sabetha,
            self.lineRaidboss_slothasor,
            self.lineRaidboss_trio,
            self.lineRaidboss_matthias,
            self.lineRaidboss_glenna,
            self.lineRaidboss_keepconstruct,
            self.lineRaidboss_twistedcastle,
            self.lineRaidboss_xera,
            self.lineRaidboss_cairn,
            self.lineRaidboss_mursaat,
            self.lineRaidboss_samarog,
            self.lineRaidboss_deimos,
            self.lineRaidboss_desmina,
            self.lineRaidboss_riverofsouls,
            self.lineRaidboss_statues,
            self.lineRaidboss_dhuum,
            self.lineRaidboss_conjureda,
            self.lineRaidboss_twinlargos,
    ...
    def load_currency_section(self):
        ...
        if self.checkCurrency.isChecked():
            if (
                "inventories" in self.api_permissions
                and "wallet" in self.api_permissions
                and "characters" in self.api_permissions
            ):
                api_characters_names = self.api_open(
                    "characters", token=self.api_key
                )
                api_characters = self.api_open(
                    "characters", ids=api_characters_names, token=self.api_key
                )
                api_shared_inventory = self.api_open(
                    "account/inventory", token=self.api_key
                )
                api_materials = self.api_open(
                    "account/materials", token=self.api_key
                )
                api_bank = self.api_open("account/bank", token=self.api_key)
                api_wallet = self.api_open(
                    "account", ids=["wallet"], token=self.api_key
                )
                self.fill_currency(
                    api_wallet,
                    api_characters,
                    api_shared_inventory,
                    api_materials,
                    api_bank,
                )
            else:
    ...
    def fill_currency(
        ...
        for i in api_wallet:
            wallet_items = [
                {
                    "name": "magnetite_shards",
                    "id": 28,
                    "uiitem": self.lineCurrency_Magnetiteshards,
                },
                {
                    "name": "gaeting_crystals",
                    "id": 39,
                    "uiitem": self.lineCurrency_Gaetingcrystals,
                },
            ]
        ...
        items_to_find = [
            {
                "name": "legendary_insight",
                "id": 77302,
                "uiitem": self.lineCurrency_Legend_insights,
            },
            {
                "name": "legendary_divination",
                "id": 88485,
                "uiitem": self.lineCurrency_Legend_divinations,
            },
        ]
    ...
    def reset_currency(self):
        ...
        # Clean everything
        currency_fields = (
            self.lineCurrency_Magnetiteshards,
            self.lineCurrency_Gaetingcrystals,
            self.lineCurrency_Legend_insights,
            self.lineCurrency_Legend_divinations,
        )
    ...
    def load_achievements_section(self):
        ...
        if self.checkAchievements.isChecked():
            if "progression" in self.api_permissions:
                api_achievs = self.api_open(
                    "account/achievements", token=self.api_key
                )
    ...
    def fill_achievements(self, api_achievs):
        ...
        # Achievements
        achievements = [
            {"id": 2657, "flag": 0, "uiitem": self.lineRaidboss_w1_1},
    ...
    def reset_achievements(self):
        ...
        # Clean everything
        achievements_fields = (
            self.lineRaidboss_w1_1,
            self.lineRaidboss_w1_2,
            self.lineRaidboss_w1_3,
            self.lineRaidboss_w1_4,
            self.lineRaidboss_w1_5,
            self.lineRaidboss_w1_6,
            self.lineRaidboss_w1_7,
            self.lineRaidboss_w1_8,
            self.lineRaidboss_w1_9,
            self.lineRaidboss_w1_10,
            self.lineRaidboss_w1_11,
            self.lineRaidboss_w1_12,
            self.lineRaidboss_w1_13,
            self.lineRaidboss_w1_14,
            self.lineRaidboss_w1_15,
            self.lineRaidboss_w1_16,
            self.lineRaidboss_w1_17,
            self.lineRaidboss_w1_18,
            self.lineRaidboss_w1_19,
            self.lineRaidboss_w1_20,
            self.lineRaidboss_w1_21,
            self.lineRaidboss_w2_1,
            self.lineRaidboss_w2_2,
            self.lineRaidboss_w2_3,
            self.lineRaidboss_w2_4,
            self.lineRaidboss_w2_5,
            self.lineRaidboss_w2_6,
            self.lineRaidboss_w2_7,
            self.lineRaidboss_w2_8,
            self.lineRaidboss_w2_9,
            self.lineRaidboss_w3_1,
            self.lineRaidboss_w3_2,
            self.lineRaidboss_w3_3,
            self.lineRaidboss_w3_4,
            self.lineRaidboss_w3_5,
            self.lineRaidboss_w3_6,
            self.lineRaidboss_w3_7,
            self.lineRaidboss_w3_8,
            self.lineRaidboss_w3_9,
            self.lineRaidboss_w3_10,
            self.lineRaidboss_w3_11,
            self.lineRaidboss_w4_1,
            self.lineRaidboss_w4_2,
            self.lineRaidboss_w4_3,
            self.lineRaidboss_w4_4,
            self.lineRaidboss_w4_5,
            self.lineRaidboss_w4_6,
            self.lineRaidboss_w4_7,
            self.lineRaidboss_w4_8,
            self.lineRaidboss_w4_9,
            self.lineRaidboss_w4_10,
            self.lineRaidboss_w4_11,
            self.lineRaidboss_w5_1,
            self.lineRaidboss_w5_2,
            self.lineRaidboss_w5_3,
            self.lineRaidboss_w5_4,
            self.lineRaidboss_w5_5,
            self.lineRaidboss_w5_6,
            self.lineRaidboss_w5_7,
            self.lineRaidboss_w5_8,
            self.lineRaidboss_w5_9,
            self.lineRaidboss_w5_10,
            self.lineRaidboss_w5_11,
            self.lineRaidboss_w6_1,
            self.lineRaidboss_w6_2,
            self.lineRaidboss_w6_3,
            self.lineRaidboss_w6_4,
            self.lineRaidboss_w6_5,
            self.lineRaidboss_w6_6,
            self.lineRaidboss_w6_7,
            self.lineRaidboss_w6_8,
            self.lineRaidboss_w6_9,
            self.lineRaidboss_w6_10,
            self.lineRaidboss_w6_11,
            self.lineRaidboss_w6_12,
    ...
    def fill_minis(self, api_minis):
        ...
        # Minis IDs
        raid_minis = [
            {"id": 371, "flag": 0, "uiitem": self.label_Mini_redguardian},
            ...
            {"id": 440, "flag": 0, "uiitem": self.label_Mini_whitemantle},
            {
                "id": 436,
                "flag": 0,
                "uiitem": self.label_Mini_ragged_whitemantle,
            },
    ...
    def reset_minis(self):
        ...
        # Clean everything
        minis_fields = (
            self.label_Mini_redguardian,
            self.label_Mini_greenguardian,
            self.label_Mini_blueguardian,
            self.label_Mini_valeguardian,
            self.label_Mini_gorseval,
            self.label_Mini_knuckles,
            self.label_Mini_kernan,
            self.label_Mini_karde,
            self.label_Mini_slubling,
            self.label_Mini_slothasor,
            self.label_Mini_berg,
            self.label_Mini_zane,
            self.label_Mini_narella,
            self.label_Mini_matthias,
            self.label_Mini_mcleod,
            self.label_Mini_keepconstruct,
            self.label_Mini_xera,
            self.label_Mini_cairn,
            self.label_Mini_mursaat,
            self.label_Mini_eyeofjanthir,
            self.label_Mini_samarog,
            self.label_Mini_whitemantle,
            self.label_Mini_ragged_whitemantle,
            self.label_Mini_desmina,
            self.label_Mini_brokenking,
            self.label_Mini_dhuum,
            self.label_Mini_zommoros,
            self.label_Mini_kenut,
            self.label_Mini_nikare,
            self.label_Mini_qadim,
        )
    ...
    def fill_skins(self, api_skins):
        ...
        # Skins IDs
        raid_skins = [
            {"id": 6528, "flag": 0, "uiitem": self.label_Skin_vg_dagger},
            ...
            {"id": 6645, "flag": 0, "uiitem": self.label_Skin_matthias_staff},
            {
                "id": 6630,
                "flag": 0,
                "uiitem": self.label_Skin_matthias_greatsword,
            },
            {"id": 6652, "flag": 0, "uiitem": self.label_Skin_matthias_longbow},
            {
                "id": 6638,
                "flag": 0,
                "uiitem": self.label_Skin_matthias_shortbow,
            },
            ...
            {"id": 7867, "flag": 0, "uiitem": self.label_Skin_statues_dagger},
            {
                "id": 7910,
                "flag": 0,
                "uiitem": self.label_Skin_statues_greatsword,
            },
            ...
            {"id": 8412, "flag": 0, "uiitem": self.label_Skin_conjured_shield},
            {
                "id": 8398,
                "flag": 0,
                "uiitem": self.label_Skin_conjured_greatsword,
            },
    ...
    def reset_skins(self):
        ...
        # Clean everything
        skins_fields = (
            self.label_Skin_vg_dagger,
            self.label_Skin_vg_greatsword,
            self.label_Skin_gorse_shield,
            self.label_Skin_gorse_staff,
            self.label_Skin_sab_rifle,
            self.label_Skin_sab_back,
            self.label_Skin_sloth_hammer,
            self.label_Skin_sloth_focus,
            self.label_Skin_matthias_staff,
            self.label_Skin_matthias_greatsword,
            self.label_Skin_matthias_longbow,
            self.label_Skin_matthias_shortbow,
            self.label_Skin_matthias_mace,
            self.label_Skin_matthias_shield,
            self.label_Skin_matthias_warhorn,
            self.label_Skin_matthias_pistol,
            self.label_Skin_matthias_torch,
            self.label_Skin_kc_hammer,
            self.label_Skin_kc_torch,
            self.label_Skin_kc_focus,
            self.label_Skin_kc_scepter,
            self.label_Skin_xera_scepter,
            self.label_Skin_xera_staff,
            self.label_Skin_xera_rifle,
            self.label_Skin_xera_sword,
            self.label_Skin_xera_axe,
            self.label_Skin_xera_dagger,
            self.label_Skin_xera_back,
            self.label_Skin_cairn_pistol,
            self.label_Skin_cairn_sword,
            self.label_Skin_mursaat_longbow,
            self.label_Skin_samarog_axe,
            self.label_Skin_samarog_shortbow,
            self.label_Skin_samarog_staff,
            self.label_Skin_samarog_warhorn,
            self.label_Skin_deimos_mace,
            self.label_Skin_deimos_hammer,
            self.label_Skin_deimos_staff,
            self.label_Skin_deimos_back,
            self.label_Skin_deimos_gloves,
            self.label_Skin_desmina_axe,
            self.label_Skin_desmina_hammer,
            self.label_Skin_river_shield,
            self.label_Skin_river_sword,
            self.label_Skin_statues_dagger,
            self.label_Skin_statues_greatsword,
            self.label_Skin_dhuum_staff,
            self.label_Skin_dhuum_helm,
            self.label_Skin_dhuum_shoulders,
            self.label_Skin_dhuum_gloves,
            self.label_Skin_dhuum_boots,
            self.label_Skin_conjured_shield,
            self.label_Skin_conjured_greatsword,
            self.label_Skin_largos_sword,
            self.label_Skin_largos_longbow,
            self.label_Skin_qadim_mace,
    ...
    def save_new_api(self):
        ...
        if not len(self.lineKey.text()) == 72:
            ...
            self.labelInfo.setText(
                "Error: The provided Key doesn't fits 72 characters."
            )
        elif self.lineName.text() == "":
            self.labelInfo.setStyleSheet("color: rgb(250, 0, 0);")
            self.labelInfo.setText(
                "Error: You must provide a name for that key."
            )
        else:
            ...
            # Add the new key
            keys.append(
                {"name": self.lineName.text(), "key": self.lineKey.text()}
            )
            # Save them into file
            INI_OPTIONS.setValue("api_keys", keys)
